...
public class DatabaseManagerGoods {
	private DatabaseHelper dbHelper = null;
	private Context context = null;

	public DatabaseManagerGoods(Context context) {
		super();
		...
		synchronized (dbHelper) {
			...
			if (db != null) {
				try {
					db.beginTransaction();
					// 数据库执行插入命令
					row = db.insert(DatabaseHelper.TABLE_NAME_GOODS,
							null, values);
			...
			if (db != null) {
				try {
					...
					String sql = "select  *  from "
							+ DatabaseHelper.TABLE_NAME_GOODS;
	...
	public void updateData(Goods goods){
		...
		String[] whereArgs = {String.valueOf(goods.getId())};//修改条件的参数
		synchronized(dbHelper) {
			SQLiteDatabase db = null;
			try {
				db = dbHelper.getWritableDatabase();
			...
			if (db != null) {
				try {
					db.beginTransaction();
					// 数据库执行插入命令
					db.update(DatabaseHelper.TABLE_NAME_GOODS, values, whereClause,whereArgs);
					db.setTransactionSuccessful();
				} catch (SQLiteException e) {
					e.printStackTrace();
				} catch (Exception e) {
